...
import sys
sys.path.append(r'C:\Users\DELL\Desktop\Toolkit\Backend') #BACKEND FILE'S FOLDER PATH #Update this as per the path of the directory(folder), where you wish to store your backend code
...
def which_set():
    if values['r1']==True:
        ...
        sg.popup_error('Select one of the datasets', font='Helvetica', text_color='yellow')
        quit()
...
def drop_window():
    ...
    layout = [
              ...
              [sg.Text('ROWS\n', font='Helvetica', text_color='yellow')],
...
def unique_sel_window(): # DISCONTINUED
    ...
    while True:
        ...
        if event=='OK':
            ...
            layout2=[
                     [sg.Text('\nUnique values in "{}" column are:\n'.format(col), font='Helvetica', text_color='yellow')],
...
def tune_window(model, algo, tune_key, X_train, y_train):
    ...
    if tune_key==2:  # KNN CLASSIFIER
        layout=[
                [sg.Text('')],
                [sg.Text('1) n_neighbors: ', text_color='yellow', font='Helvetica'),
                 # int
...
def metric_selection_window():
    ...
    layout=[
            ...
            [sg.Text('--'*30)],
            [sg.Text('Classification metrics:', font='Helvetica', text_color='yellow')],
            [sg.Combo(classification_metrics, enable_events=True, key='cm_combo')],
            [sg.Button('OK', key='OK_cm')],
            [sg.Text('--'*30)],
            [sg.Text('Regression metrics:', font='Helvetica', text_color='yellow')],
...
def metric_display(result_train, result_val, mf):
    if mf=='c1': # Confusion Matrix FORMAT DISCONTINUED
        ...
        layout = [
                  [sg.Text('\nConfusion Matrix:', font='Helvetica', text_color='yellow')],
...
def bar_plot():
    ...
    num_cols, cat_cols =tb.num_cat_cols(df)
    '''layout=[
            [sg.Text('\nParameter on X-axis:', text_color='yellow', font='Helvetica')],
            ...
            [sg.Text('--'*45)],
            [sg.Text('\nParameter on Y-axis:', text_color='yellow', font='Helvetica')],
...
def heatmap_plot():
    df, data, header_list, fn, r1=which_set()
    layout=[
            [sg.Text('\nCreate Heatmap for:', font='Helvetica', text_color='yellow')],
...
listbox_name = [
                [sg.Text('The entered files are as follows:', text_color='yellow', font='Helvetica')],
                [sg.Text('', key='t1', size=(30,1))],
                [sg.Text('', key='t2', size=(30,1))],
                [sg.Text('\nSelect one:', text_color='yellow' , font='Helvetica')],
...
left_super_col=[
                ...
                [sg.Column(buts_col),
                 sg.VSeperator(),
                 sg.Column(listbox_name),
                 sg.VSeperator(),
                 sg.Column(cleaning_col)]
               ]

plot_col = [
            [sg.Text('Select a plotting method for visualising the selected dataset')],
            [sg.Button('Line'),
             sg.Button('Bar')],
            [sg.Button('Heatmap'),
             sg.Button('Scatter')],
...
layout = [
          [sg.Column(left_super_col),
           sg.VSeperator(),
           sg.Column(plot_col)],
          [sg.Text('_'*100)],
          [sg.Column(apply_model_col)]

         ]
# Final layout END...

window = sg.Window('Try_v1', layout, finalize=True)
window.maximize()
